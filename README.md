...
- gitleaks の release tarball から **バイナリを直接 install** して実行 (`gitleaks/gitleaks` MIT)
- 1 vCPU の **`ubuntu-slim`** ランナーで実行 (15 分上限の軽量ランナー)
- 公式 Docker image (`ghcr.io/gitleaks/gitleaks`) も推奨方式の一つだが、`ubuntu-slim` には Docker daemon が無いためバイナリ install 方式を採用
